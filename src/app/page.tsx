      ...
      <section className="flex flex-col justify-center items-start min-w-full h-0 flex-1 px-10">
        <AnimatePresence>
          {!isFocused && (
            ...
            >
              <h1 className="text-black text-5xl font-mono w-full h-full">
        ...
        <motion.form
          ...
          className={`flex w-full mt-5 ${
            isFocused ? "absolute top-20 right-10 w-1/3" : "relative"
          }`}
          initial={isFocused ? { scale: 0.9, x: 0 } : { scale: 1, x: 0 }}
          animate={isFocused ? { scale: 1, x: 0 } : { scale: 1, x: 0 }}
          transition={{ duration: 0.5 }}
        >
          <input
            {...register("input")}
            type="text"
            className={`w-full h-12 rounded border border-[#9888FF] focus:outline-none p-5 bg-[#ECECFF] text-xl ${
